...
function openImageModal(index) {
    if (galleryImagesData.length === 0) {
        console.warn('Galerie je prázdná!');
        return;
    }
...
function debugGallery() {
    ...
    if (galleryImagesData.length > 0) {
        ...
        console.log(`Předchozí: index ${prevIndex} (${galleryImagesData[prevIndex]?.name || 'N/A'})`);
        console.log(`Další: index ${nextIndex} (${galleryImagesData[nextIndex]?.name || 'N/A'})`);
    }
...
//tady je logika pro vkládání url adres pro moje projekty selection contacs div cloude projekt test
